...
from config import CLASSIFIER_RESULT_FNAME, CLASSIFIER_RESULT_FNAME_TEXT, CLASSIFIER_RESULTS_FOLDER, MAX_L, MIN_L

TRAINING_SIZES = [48, 24, 16, 8]

CLASSIFIERS = ['linear', 'rbf', 'poly', 'sigmoid', 'random_forest']
...
def export_results(results: list, training_size: int, threshold: int):
    ...
    export_path = CLASSIFIER_RESULTS_FOLDER / CLASSIFIER_RESULT_FNAME.format(training_size=training_size, l=threshold, time_index=NOW_TIME_INDEX)
...
def export_results_str(results: list, training_size: int, threshold: int):
    export_path = CLASSIFIER_RESULTS_FOLDER / CLASSIFIER_RESULT_FNAME_TEXT.format(training_size=training_size, l=threshold, time_index=NOW_TIME_INDEX)
    export_path.parent.mkdir(parents=True, exist_ok=True)
