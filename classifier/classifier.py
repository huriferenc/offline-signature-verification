...
class Classifier:
    ...
    def __init__(self, training_size: int, threshold: int, clf_type: CLF_TYPE) -> None:
        ...
        if self.is_kernel:
            self.param_grid = {"C": self.C, "kernel": [self.clf_type]}
...
"""
    ...
    def get_original_signature_dfs(self, image_number, threshold, use_cols=[]):
        ...
        for i in range(image_number):
            df = pd.read_csv(
                EXTRACTED_FEATURE_SET_SAVING_FOLDER
                / DATAFRAME_FNAME_ORIG.format(sign_index=i + 1, l=threshold),
                usecols=use_cols,
            )

            arr = np.append(arr, {"X": df.to_numpy(), "y": 1})
    ...
    def get_forgery_signature_dfs(self, image_number, threshold, use_cols=[]):
        ...
        for i in range(image_number):
            df = pd.read_csv(
                EXTRACTED_FEATURE_SET_SAVING_FOLDER
                / DATAFRAME_FNAME_FORG.format(sign_index=i + 1, l=threshold),
                usecols=use_cols,
            )

            arr = np.append(arr, {"X": df.to_numpy(), "y": 0})
    ...
    def run(self):
        ...
        print(
            f"Running {self.clf_type.capitalize()}{' SVM' if self.is_kernel else ''} Classifier...\n"
        )
        ...
        ###########################################################################################
        origin_dfs = self.get_original_signature_dfs(
            IMAGE_NUMBER, self.threshold, USE_COLS
        )
        forgery_dfs = self.get_forgery_signature_dfs(
            IMAGE_NUMBER, self.threshold, USE_COLS
        )
        ...
        self.best_accuracy = classification_report(
            y_true=y_test, y_pred=grid_predictions, output_dict=True
        )
        self.best_accuracy_str = classification_report(
            y_true=y_test, y_pred=grid_predictions, output_dict=False
        )
        ...
        self.confusion_matrix = confusion_matrix(
            y_true=y_test, y_pred=y_pred, labels=[1, 0]
        )
