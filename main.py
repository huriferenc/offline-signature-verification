...
class App:
    ...
    def getDataFrame(self, class_segments, P):
        ...
        for i, item in enumerate(class_segments):
            ...
            same_pixel_condition = np.logical_and(
                np.logical_or(DQSSi_img == START_SEGMENT, DQSSi_img == SEGMENT),
                np.logical_or(
            ...
            regions = [
                ...
                DQSSi_img[part_height : 2 * part_height, part_width : 2 * part_width],
                DQSSi_img[
            ...
            for index, region in enumerate(regions):
                pixels_count = np.sum(
                    np.logical_or(region == START_SEGMENT, region == SEGMENT)
                )
    ...
    def run(self):
        ...
        for sign_id in range(IMAGE_NUMBER):
            ...
            for l in range(MIN_L, MAX_L + 1):
                ...
                class_segments_original, P_original = self.process_edges(
                    l, filename_original, SIGN_FOLDER_ORIG, saving_folder_original
                )
                df_original = self.getDataFrame(class_segments_original, P_original)
                self.export_feature_set(
                    f"{filename_original}.L_{l}.csv", efs_saving_folder, df_original
                )

                class_segments_forgery, P_forgery = self.process_edges(
                    l, filename_forgery, SIGN_FOLDER_FORG, saving_folder_forgery
                )
                df_forgery = self.getDataFrame(class_segments_forgery, P_forgery)
                self.export_feature_set(
                    f"{filename_forgery}.L_{l}.csv", efs_saving_folder, df_forgery
                )
